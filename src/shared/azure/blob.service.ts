...
export default class BlobService {
  ...
  async deleteBlobAsync(containerName: string, blobName: string): Promise<void> {
    ...
    try {
      ...
      this.loggerSvc.error(`BlobService.deleteBlobAsync: ${error.toString()}`);
    }
  }
  ...
  async uploadBlobAsync(containerName: string, file: Multipart): Promise<string | undefined> {
    ...
    try {
      ...
      this.loggerSvc.error(`BlobService.uploadBlobAsync: ${error.toString()}`);
    }
  ...
  async uploadBlobsAsync(
    ...
    try {
      ...
      this.loggerSvc.error(`BlobService.uploadBlobsAsync: ${error.toString()}`);
    }
  ...
  async createContainerAsync(containerName: string): Promise<boolean> {
    ...
    try {
      ...
      this.loggerSvc.error(`BlobService.createContainerAsync: ${error.toString()}`);
    }
